...
class Trie {
 ...
 public:
  ...
  bool RemoveHelper(
    ...
    std::unique_ptr<TrieNode>* current_node, 
    std::unique_ptr<TrieNode>* prev_node) 
  {
    if (index == key.size()) { // current_node is the end node
      ...
      if (!(*current_node)->HasChild(key[index]))
        return false;
      
      // Removing substring failed
      if (!RemoveHelper(key, index + 1, (*current_node)->GetChildNode(key[index]), current_node))
        return false;
    }
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    ...
    if (key.empty())
      return {}; // Return a value-initialized instance of the function's return type
