...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    latch_.WLock();
    auto current_node = &root_;  // TrieNode**, std::unique_ptr<TrieNode>*

    // Move to the end node, construct it if not exists
    for (size_t i = 0; i < key.size(); i++) {
      if (!(*current_node)->HasChild(key[i])) {
        // std::make_unique<TrieNode>(key[i]) returns an rvalue
    ...
    // Convert current_node to a TrieNodeWithValue
    // The reset method of unique_ptr deletes the old object and assigns the new one
    current_node->reset(new TrieNodeWithValue(std::move(**current_node), value));
  ...
  bool Remove(const std::string &key) {
    ...
    bool result = RemoveHelper(key, 0, &root_, nullptr);

    latch_.WUnlock();
    return result;
  }

  bool RemoveHelper(const std::string &key, size_t index, std::unique_ptr<TrieNode> *current_node,
                    std::unique_ptr<TrieNode> *prev_node) {
    if (index == key.size()) {  // current_node is the end node
      if (!(*current_node)->IsEndNode()) return false;

      (*current_node)->SetEndNode(false);
    } else {
      // current_node does not contain the next expected char
      if (!(*current_node)->HasChild(key[index])) return false;

      // Removing substring failed
      if (!RemoveHelper(key, index + 1, (*current_node)->GetChildNode(key[index]), current_node)) return false;
    }

    // Now check if current_node should be removed
    if (!(*current_node)->HasChildren() && !(*current_node)->IsEndNode()) {
      if (prev_node && *prev_node) {                    // prev_node is nullptr in the first call
        (*prev_node)->RemoveChildNode(key[index - 1]);  // Because index has been incremented
      }
    }
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    ...
    if (key.empty()) return {};  // Return a value-initialized instance of the function's return type
    ...
    for (char ch : key) {
      ...
      current_node = (*current_node)->GetChildNode(ch);
    }

    // Use dynamic_cast to check end node
    auto end_node = dynamic_cast<TrieNodeWithValue<T> *>(current_node->get());
